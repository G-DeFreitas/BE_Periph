...
void MyUSART_Init (MyUART_Struct_TypeDef * MyUart) {
	...
	MyUart->Uart->CR1 &= ~USART_CR1_M; //On aura 1 bit de Start, 8 bits de données
	MyUart->Uart->CR2 &= ~USART_CR2_STOP; //!!!!!!!!On choisit de mettre STOP à 00, pour un 1 bit de stop
...
void USART1_IRQHandler(void){
	USART1->SR &= ~USART_SR_RXNE; // On remet le flag à 0
	//Message = USART1->DR;
	if (HandlerUart1 !=0){
	(*HandlerUart1) ();
	}
...
void USART2_IRQHandler(void){
	USART2->SR &= ~USART_SR_RXNE; // On remet le flag à 0
	//Message = USART2->DR;
	if (HandlerUart2 !=0){
	(*HandlerUart2) ();
	}
...
void USART3_IRQHandler(void){
	USART3->SR &= ~USART_SR_RXNE; // On remet le flag à 0
	//Message = USART3->DR;
	if (HandlerUart3 !=0){
	(*HandlerUart3) ();
	}
